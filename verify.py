...
# ══════════════════════════════════════════════════════════
#  STEP 15: YAML Error Line Numbers
# ══════════════════════════════════════════════════════════
header("STEP 15: YAML Error Line Numbers")

resp = submit_pipeline("test_pipelines/bad-lines.yaml", token)
check("Invalid YAML → 422", resp.status_code == 422, f"got {resp.status_code}")
if resp.status_code == 422:
    detail = resp.json().get("detail", "")
    check("Error contains line number 5", "Line 5" in detail, f"got: {detail}")


# ══════════════════════════════════════════════════════════
#  STEP 16: Shared Job Workspaces
# ══════════════════════════════════════════════════════════
header("STEP 16: Shared Job Workspaces")

resp = submit_pipeline("test_pipelines/shared-workspace.yaml", token)
run_id = resp.json().get("run_id")
print(f"  Run ID: {run_id}")

data = wait_for_run(run_id)
check("Shared workspace pipeline → succeeded", data["status"] == "succeeded", f"got {data['status']}")
check("job-a (write) → succeeded", data["jobs"].get("job-a", {}).get("status") == "succeeded")
check("job-b (read shared file) → succeeded", data["jobs"].get("job-b", {}).get("status") == "succeeded")
